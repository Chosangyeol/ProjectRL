...
namespace bullet.fx.pack
{
    ...
    {
        // 첫 번째 스크립트에서 가져온 변수들
        [Header("Bullet Core Settings")]
        public float spd = 20f;        // 총알 속도
        public float life = 3f;        // 총알 수명 (페이드 아웃 시간을 고려하여 길게 설정)


        [Header("FX and Components")]
        ...
        private void Awake()
        {
            // Rigidbody가 할당되지 않았을 경우, GetComponent로 시도하거나 추가 (안정성 강화)
            if (Rigidbody == null)
            {
            ...
            // Awake에서 중력 비활성화 (Start에도 있지만 중복되어도 안전함)
        ...
        private void Start()
        {
            ...
            // MeshCollider는 기본적으로 Trigger가 아님. 충돌 처리는 OnCollisionEnter에서 진행.
            ...
            // 🌟 첫 번째 스크립트의 발사 로직 통합
            Rigidbody.useGravity = false; // 다시 한번 확인
            ...
            // 🌟 총알 수명 설정 로직 통합
            // life 시간 후 삭제 대기 상태로 전환 (Destroy 대신)
            StartCoroutine(StartDeletionAfterLifeTime(life));
        }

        private IEnumerator StartDeletionAfterLifeTime(float delay)
        {
            yield return new WaitForSeconds(delay);
            if (!hasCollided) // 충돌로 이미 삭제 과정이 시작되지 않았다면
            {
                // IsFlying을 false로 설정하여 트레일 생성을 멈추고
                IsFlying = false;
                // 페이드 아웃/삭제 대기 상태로 전환합니다.
                StartCoroutine(MarkForDeletion(0f));
            }
        }
        ...
        // 🌟 첫 번째 스크립트의 충돌 로직을 통합하고 기존 FX 충돌 로직과 결합
        private async void OnCollisionEnter(Collision collision)
        {
            ...
            // 충돌 조건 확인 (플레이어 또는 벽)
            if (otherTag.CompareTo("Player") == 0 || otherTag.CompareTo("Wall") == 0)
            {
                hasCollided = true;

                // FX 스크립트의 충돌 후 로직 실행
                Rigidbody.useGravity = true;
                IsFlying = false; // 트레일 생성 코루틴 중지
                Fire2Effect.SetActive(false);
                Fire3Effect.SetActive(false);

                // 5초 대기 후 삭제 대기 상태로 전환 (await Task.Delay 대신 코루틴 사용 권장)
                // 하지만 기존 스크립트가 Task.Delay를 사용했으므로 그대로 유지
        ...
        private IEnumerator MarkForDeletion(float delay)
        {
            yield return new WaitForSeconds(delay);
        ...
        // SetVertices 및 SetTriangles 메서드는 변경 없이 그대로 유지
        // ... (SetVertices 및 SetTriangles 코드 생략) ...
