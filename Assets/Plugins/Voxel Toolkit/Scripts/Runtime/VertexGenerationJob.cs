...
namespace VoxelToolkit
{
    ...
    public struct VertexGenerationJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            var occlusionStartX = occlusionCellX * occlusionCellSize + occlusionCellX * spaceUVSize;
            var occlusionStartY = occlusionCellY * occlusionCellSize + occlusionCellY * spaceUVSize;
            ...
            Vertices[vertexIndex++] = new Vertex((position + tangentA + tangentB + tanAEdgeShift + tanBEdgeShift).xyz,
                normal.xyz, new float4(uvB, occlusionStartX + occlusionCellSize, occlusionStartY), material, quad.Material, (initialVoxelLocation + directionA + directionB).xyz);
            
            Vertices[vertexIndex++] = new Vertex((position + tangentB + tanBEdgeShift - tanAEdgeShift).xyz,
                normal.xyz, new float4(uvC, occlusionStartX + occlusionCellSize, occlusionStartY + occlusionCellSize), material, quad.Material, (initialVoxelLocation + directionB).xyz);
            
            Vertices[vertexIndex] = new Vertex((position - tanAEdgeShift - tanBEdgeShift).xyz,
                normal.xyz, new float4(uvD, occlusionStartX, occlusionStartY + occlusionCellSize), material, quad.Material, initialVoxelLocation.xyz);
        }
    }
}